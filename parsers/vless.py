...
def parse(data):
    ...
    if netquery.get('type'):
        if netquery['type'] == 'http':
            ...
            node['transport'] = {
                ...
                'headers': {
                    'Host': '' if netquery.get('host') is None and netquery.get('sni') == 'None'
                    else netquery.get('host', netquery.get('sni', ''))
                }
            }

            if node.get('tls') and node['tls']['server_name'] == '' and node['transport']['headers']['Host']:
                node['tls']['server_name'] = node['transport']['headers']['Host']

            if matches:
                node['transport']['early_data_header_name'] = 'Sec-WebSocket-Protocol'
                node['transport']['max_early_data'] = int(
                    netquery.get('path', '/').rsplit('?ed=', 1)[1]
                )
        ...
        # ===== 新增 xhttp =====
            ...
            node['transport'] = {
                ...
                'headers': {
                    'Host': netquery.get('host', netquery.get('sni', ''))
                }
            }
    ...
    # ================= Shadowrocket obfs =================
        ...
        if netquery['obfs'] == 'websocket':
            ...
            node['transport'] = {
                ...
                'path': netquery.get('path', '/').rsplit('?ed=', 1)[0] if matches else netquery.get('path', '/'),
                'headers': {
                    'Host': '' if netquery.get('obfsParam') is None and netquery.get('sni') == 'None'
                    else netquery.get('peer', netquery.get('obfsParam'))
                }
            }
    ...
    if netquery.get('protocol') in ['smux', 'yamux', 'h2mux']:
        ...
        if netquery.get('max-streams'):
            ...
            node['multiplex']['max_connections'] = int(netquery.get('max-connections', 0))
            node['multiplex']['min_streams'] = int(netquery.get('min-streams', 0))
